FROM python:3.9 AS silicoin_build
...
RUN \
	...
	&& git clone --branch ${RELEASE} --recurse-submodules=mozilla-ca https://github.com/silicoin-network/silicoin-blockchain.git . \
	&& /bin/sh ./install.sh
...
ENV \
        SILICOIN_ROOT=/root/.sit/mainnet \
        farmer_address= \
        farmer_port= \
        keys="generate" \
        log_level="INFO" \
...
# copy build files
COPY --from=silicoin_build /sit-blockchain /sit-blockchain
